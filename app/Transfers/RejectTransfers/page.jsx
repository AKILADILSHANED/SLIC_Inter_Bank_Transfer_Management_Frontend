...
export default function RejectTransfers() {
    ...
    //Define handleRejectTransfer function;
    const handleRejectTransfer = async (transferId) => {
        try {
            const request = await fetch(
                `${baseUrl}/api/v1/transfers/update-rejection?transferId=${transferId}`,
                {
                    method: "PUT",
                    credentials: "include"
                }
                                        ...
                                        transferData.map((element) => (
                                            <tr key={element.transferId} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap flex items-center justify-center">
                                                    <button onClick={()=>handleRejectTransfer(element.transferId)} type="button" className="text-red-700 shadow-md h-[30px] w-[80px] hover:text-white border border-red-700 hover:bg-red-700 font-medium rounded-md text-md  text-center me-2 mb-2 dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-500 dark:focus:ring-red-800">Reject</button>
                                                </td>
                                            </tr>
                                        ))
                                    }
