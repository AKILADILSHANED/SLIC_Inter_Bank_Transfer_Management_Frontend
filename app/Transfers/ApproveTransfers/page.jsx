...
export default function ApproveTransfers() {
    ...
    const viewTransferData = async (e) => {
        ...
        try {
            ...
            const request = await fetch(
                `${baseUrl}/api/v1/transfers/approve-transfers?transferDate=${encodeURIComponent(transferDate)}`,
                {
                    method: "GET",
                    credentials: "include",
                }
    ...
    //Define handleApproveTransfer function;
    const handleApproveTransfer = async (transferId) => {
        try {
            const request = await fetch(
                `${baseUrl}/api/v1/transfers/update-approval?transferId=${transferId}`,
                {
                    method: "PUT",
                    credentials: "include"
                }
                                ...
                                transferData.map((element) => (
                                    <tr key={element.transferId} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap flex items-center justify-center">
                                            <button onClick={()=>handleApproveTransfer(element.transferId)} type="button" className="text-green-700 shadow-md h-[30px] w-[80px] hover:text-white border border-green-700 hover:bg-green-800 font-medium rounded-md text-md  text-center me-2 mb-2 dark:border-green-500 dark:text-green-500 dark:hover:text-white dark:hover:bg-green-500 dark:focus:ring-green-800">Approve</button>
                                        </td>
                                    </tr>
                                ))
                            }
