        ...
        const fetchTransferDetails = async () => {
            ...
            try {
                const request = await fetch(
                    `${baseUrl}/api/v1/printing/ibt-letter-details?voucherNo=${voucherId}`,
                    {
                        method: "GET",
                        credentials: "include"
                    }
                        ...
                        <div className="printable-area  text-[12pt] leading-relaxed text-black">
                            ...
                            <section className='mt-8 font-bold'>
                                <p className='underline decoration-1 underline-offset-4'>
                                    Transfer of LKR. {letterData.transferAmount?.toLocaleString('en-US', {
                                        minimumFractionDigits: 2,
                                        maximumFractionDigits: 2
                                    })}
                                    {" "} from {letterData.fromBank}, {letterData.fromBranch}, A/C No. {letterData.fromAccount} to {letterData.toBank}, {letterData.toBranch}, A/C No. {letterData.toAccount} of Sri Lanka Insurance Corporation General Limited.
                                </p>
                            </section>

                            <section className='mt-8'>
                                <p className="text-justify">
                                    Please be kind enough to transfer of <label className='font-bold'>{amountToWords(letterData.transferAmount)} (LKR. {letterData.transferAmount?.toLocaleString('en-US', {
                                        minimumFractionDigits: 2,
                                        maximumFractionDigits: 2
                                    })})</label>
                                    ...
                                    value date on <label className='font-bold'>{letterData.valueDate}</label>, under advice to us.
                                </p>
                            </section>
                            ...
                            <section className='mt-28 flex flex-row justify-between'>
